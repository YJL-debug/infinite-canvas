...
+ [新增] 图片组支持用同一句要求逐张批量改图，结果另存新组，每张保留对应原图用于重试。

+ [新增] 画布文生图支持随机人物与画风，每张结果保存独立设定和实际提示词，重试沿用该图设定。

+ [修复] Grok 视频生成改用原生 JSON 接口，正确传递参考图片、创建任务并获取结果，刷新后可继续查询。

+ [修复] 反推移除自写任务描述与 JSON 输出指令，恢复 Skill 原始 Markdown 输出；生成前可先选择模型和推理强度，结果记录实际使用的参数。

+ [新增] 图片反推接入 GPT-Image2-Skill 规则，自动生成可编辑的中英文提示词、负面词及相连的生图配置。
